  ...
  if (existingProductIndex !== -1) {
    ...
    if (event?.product?.sale_tax_including) {
      if (this.userType == 'Employee') {
        ...
        // this.TotalWithoutTax[index] = getCoastPrice * 1 || 1;
        let taxPrice;
        taxPrice = (getCoastPrice * this.apiPurchaseTax) / 100;
        this.taxPrice = (getCoastPrice * this.apiPurchaseTax) / 100;
        this.TotalWithoutTax[index] = ((getCoastPrice * 1) - (taxPrice * 1)).toFixed(2) || 1;
      ...
      // this.TotalWithoutTax[index] = getCoastPrice * 1 || 0;
...
// Initialize discount status for each index
        ...
        const saleEstimateCart = (this.saleEstimateForm.get('estimate_cart') as FormArray).at(index) as FormGroup
        saleEstimateCart?.controls['discount'].setValue('');
        ...
        if(this.totalFlatDiscountPercentage > 0) {
          saleEstimateCart?.controls['discount'].setValue(this.totalFlatDiscountPercentage);
        } else {
          saleEstimateCart?.controls['discount'].setValue(this.totalFlatDiscountAmount);
        }
      }
    }
    // });
