...
@Component({
  selector: 'app-update-delivery-challan',
  templateUrl: './update-delivery-challan.component.html',
  styleUrls: ['./update-delivery-challan.component.scss']
})
export class UpdateDeliveryChallanComponent implements OnInit {
...
//end 21-5
  ...
  submit(type: any) {
    ...
    if (this.deliveryChallanForm.valid) {
      ...
      this.saleService.updateDelivryChallan(formdata,this.id).subscribe(res => {
        // console.log(res);
        this.getRes = res;
        if (this.getRes.success) {
          if (type == 'new') {
            ...
            this.router.navigate(['//sales/detail-delivery-challan/' + this?.id]);
          } else if (type == 'draft') {
            this.toastrService.success(this.getRes.msg);
