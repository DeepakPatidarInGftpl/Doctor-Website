...
@Component({
  selector: 'app-update-advance-booking',
  templateUrl: './update-advance-booking.component.html',
  styleUrls: ['./update-advance-booking.component.scss']
})
export class UpdateAdvanceBookingComponent implements OnInit {
  ...
  udateCart(add: any): FormArray {
    console.log(add);
    this.addCart();
    const formArr = this.saleEstimateForm.get('advance_booking_cart') as FormArray;
    add.forEach((j: any, i) => {
      ...
      let TotalWithoutTax = j.price - taxPrice;
      this.TotalWithoutTax[i] = (TotalWithoutTax * j.qty).toFixed(2);
      ...
      this.saleService.updateAdvanceBooking(formdata, this.id).subscribe(res => {
        ...
        if (this.getRes.success) {
          if (type == 'new') {
            ...
            this.router.navigate(['//sales/detail-advance-booking/' + this.id])
          } else if (type == 'draft') {
