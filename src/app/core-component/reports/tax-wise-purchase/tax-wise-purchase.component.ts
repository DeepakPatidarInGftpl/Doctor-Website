...
@Component({
  selector: 'app-tax-wise-purchase',
  templateUrl: './tax-wise-purchase.component.html',
  styleUrls: ['./tax-wise-purchase.component.scss']
})
export class TaxWisePurchaseComponent implements OnInit {
  loader = true;
  ...
  getPurchaseRegister() {
    this.reportService.getTaxWisePurchase(this.startDate, this.endDate, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
 ...
 async generatePDFAgain() {
  ...
  const permanentContent = 'Tax Wise Purchase Report';
  doc.setFontSize(16);
  ...
  doc.text(`To Date: ${this.formatDate(this.purchaseRegisterForm.get('end').value)}`, (pageWidth - textWidth), 45);
    ...
    doc.save('Tax_wise_Purchase .pdf');
  }
  ...
  exportToExcel(): void {
    ...
    const fileName = 'stockmovement.xlsx';
