...
@Component({
  selector: 'app-sale-item-register',
  templateUrl: './sale-item-register.component.html',
  styleUrls: ['./sale-item-register.component.scss']
})
export class SaleItemRegisterComponent implements OnInit {
  ...
  getproductDayBook() {
    this.reportService.getSaleItemRegister(this.startDate, this.endDate, this.fyID, this.selectData).subscribe((res: any) => {
      console.log(res);
 ...
 async generatePDFAgain() {
  ...
  const permanentContent = 'Sale Item Register Report';
  doc.setFontSize(16);
  ...
  doc.text(`To Date: ${this.formatDate(this.productDayBookform.get('end').value)}`, (pageWidth - textWidth+20), 45);
    // Pass tableData to autoTable
    // Pass tableData to autoTable
    autoTable(doc, {
      head: [
        ['#', 'BillDate', 'CustomerBillNo.', 'Gst', 'Barcode', 'ItemName', 'Qty', 'Price', 'TaxableAmount', 'TotalTax', 'Discount', 'AdditionalDiscount', 'Tax', 'Total']
      ],
      body: this.productDayBookList.map((row: any, index: number) => [
        index + 1,
        this.formatDate(row.bill.bill_date),
        row.bill.customer_bill_no,
        row.bill.gst,
        row.data.barcode,
        row.data.item_name,
        row.data.qty,
        row.data.price,
        row.data.taxtable_amount.taxtable_amounnt,
        row.data.taxtable_amount.total_tax.toFixed(2),
        row.data.discount,
        row.data.additional_discount,
        row.data.tax,
        row.data.total,
    ...
    doc.save('Sale_Item_Register.pdf');
  }
