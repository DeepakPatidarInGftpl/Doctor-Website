...
@Component({
  selector: 'app-brand-wise-sale',
  templateUrl: './brand-wise-sale.component.html',
  styleUrls: ['./brand-wise-sale.component.scss']
})
export class BrandWiseSaleComponent implements OnInit {
  loader = true;
  ...
  ngOnInit(): void {
    ...
    // Brand Wise Sale form
    this.brandWiseSaleForm = new FormGroup({
    ...
    this.commonService.validateAndClearDates(this.brandWiseSaleForm, this.minDate, this.maxDate);

    this.startDate = this.brandWiseSaleForm.value?.start;
    this.endDate = this.brandWiseSaleForm.value?.end;
    this.brand = this.brandWiseSaleForm.value?.brand;
    this.getBrandWiseSale();
    this.getBrand();
  ...
  search() {
    if (this.titlee === "") {
      ...
      this.brandWiseSaleList = this.brandWiseSaleList.filter((res: any) => {
  ...
  getBrandWiseSale() {
    ...
    this.reportService.getBrandWiseSale(this.startDate, this.endDate, this.brand, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
      this.brandWiseSale = res;
      this.brandWiseSaleList = res;
    })
  ...
  oncheckBrand(data: any) {
    console.log(data);
    this.dataId = data;
    this.brandWiseSaleForm.patchValue({ brand: this.dataId });
    console.warn(this.brandWiseSaleForm.value);
    this.brand = this.brandWiseSaleForm.value?.brand;
    this?.getBrandWiseSale();
  }


  getSelectedBrandWiseSaleDates() {
    console.log(this.brandWiseSaleForm.value);
    const start = this.datepipe.transform(this.brandWiseSaleForm.value.start, 'yyyy-MM-dd');
    const end = this.datepipe.transform(this.brandWiseSaleForm.value.end, 'yyyy-MM-dd');
    console.log(start);
    console.log(end);
...
async generatePDFAgain() {
   ...
   doc.setFontSize(25);
   doc.text('Brand Wise Sale Report', 52, 25);
   // Add details
   doc.setFontSize(12);
   doc.text(`Business Location: ${result?.branch}`, 14, 39);
   doc.text(`From Date: ${this.formatDate(this.brandWiseSaleForm.get('start').value)}`, 14, 45);
   doc.text(`User: ${result?.role}`, 172, 33);
   doc.text(`Print Date: ${printDate}`, 153, 39);
   doc.text(`To Date: ${this.formatDate(this.brandWiseSaleForm.get('end').value)}`, 157, 45);//,argin x, y
   autoTable(doc, {
    head: [
      ['#', 'Name', 'Total Qty.', 'Total Amount']
    ],
    body: this.brandWiseSaleList.map((row: any, index: number) => [
      index + 1,
      row.name,
      row.total_qty,
      row.total_amount,
