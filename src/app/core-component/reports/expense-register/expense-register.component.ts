...
export class ExpenseRegisterComponent implements OnInit {
  ...
  productDayBookList:any
    getproductDayBook() {
      this.reportService.getExpenseRegister(this.startDate, this.endDate,this.fyID,this.selectData).subscribe((res:any) => {
        console.log(res);
    ...
    generatePDFAgain() {
      const doc = new jsPDF();
      const subtitle = 'PV';
      const title = 'Expense Register Report';
      ...
      autoTable(doc, {
        head: [
          ['#', 'Party', 'Expense Date', 'Expense No.', 'Refernce Bill No.','Tax Amount','Total Amount']
        ],
    ...
    exportToExcel(): void {
      ...
      const fileName = 'expenseregister.xlsx';
