...
export class StockVerificationComponent implements OnInit {
  ...
  getStockAlert() {
    ...
    this.reportService.getStockVerification(this.startDate, this.endDate, this.variant,this.fyID,this.selectData).subscribe((res) => {
      console.log(res);
      this.stockAlert = res;
 ...
 generatePDFAgain() {
  const doc = new jsPDF();
  const subtitle = 'PV';
  const title = 'Stock Verifcation Report';
  ...
  autoTable(doc, {
    head: [
      ['#', 'VariantName','Sku', 'ItemCode','ItemName','System Qty','Physical Qty','Loss Qty']
    ],
    body: this.stockAlertList.map((row:any, index:number ) => [
      index + 1,
      row.barcode.variant_name,
      row.barcode.sku,
      row.item_code,
      row.item_name,
      row.system_qty,
  ...
  doc.save('Stock _Verfication .pdf');
}
...
exportToExcel(): void {
  ...
  const fileName = 'Stockverification.xlsx';
