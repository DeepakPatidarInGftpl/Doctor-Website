...
@Component({
  selector: 'app-tax-wise-credit-note',
  templateUrl: './tax-wise-credit-note.component.html',
  styleUrls: ['./tax-wise-credit-note.component.scss']
})
export class TaxWiseCreditNoteComponent implements OnInit {
  loader = true;
  ...
  getPurchaseRegister() {
    this.reportService.getTaxWiseCreditNote(this.startDate, this.endDate, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
 ...
 async generatePDFAgain() {
  ...
  const permanentContent = 'Tax Wise Credit Note Report';
  doc.setFontSize(16);
  ...
  doc.text(`To Date: ${this.formatDate(this.purchaseRegisterForm.get('end').value)}`, (pageWidth - textWidth+20), 45);
    ...
    this.purchaseRegisterList.forEach((list: any) => {
      ...
      const Name = list.name.party_name;
    ...
    doc.save('Tax_wise_Credit_Note .pdf');
  }
  ...
  exportToExcel(): void {
    ...
    const fileName = 'taxwisecreditnote.xlsx';
