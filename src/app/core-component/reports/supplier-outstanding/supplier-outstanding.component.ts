...
export class SupplierOutstandingComponent implements OnInit {
  ...
  getPurchaseRegister() {
    this.reportService.getSupplierOutstanding(this.startDate, this.endDate, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
 ...
 async generatePDFAgain() {
  ...
  doc.text(`To Date: ${this.formatDate(this.purchaseRegisterForm.get('end').value)}`, (pageWidth - textWidth), 45);
    ...
    autoTable(doc, {
      head: [
        ['#', 'User Detail', 'Debit', 'Credit', 'Closing']
      ],
    ...
    doc.save('Supplier_Outstanding.pdf');
  }
  ...
  exportToExcel(): void {
    ...
    const fileName = 'purchaseRegister.xlsx';
