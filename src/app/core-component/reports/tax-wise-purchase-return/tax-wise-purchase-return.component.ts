...
@Component({
  selector: 'app-tax-wise-purchase-return',
  templateUrl: './tax-wise-purchase-return.component.html',
  styleUrls: ['./tax-wise-purchase-return.component.scss']
})
export class TaxWisePurchaseReturnComponent implements OnInit {
  loader = true;
  ...
  getPurchaseRegister() {
    this.reportService.getTaxWisePurchaseReturn(this.startDate, this.endDate, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
 ...
 async generatePDFAgain() {
  ...
  const permanentContent = 'Tax Wise Purchase Return Report';
  doc.setFontSize(16);
  ...
  doc.text(`To Date: ${this.formatDate(this.purchaseRegisterForm.get('end').value)}`, (pageWidth - textWidth), 45);
    ...
    autoTable(doc, {
      ...
      startY: 49,
      headStyles: {
        fillColor: [255, 159, 67], // Header background color
    ...
    doc.save('Tax_wise_Purchase_Return  .pdf');
  }
  ...
  exportToExcel(): void {
    ...
    const fileName = 'taxwisepurchasereturn.xlsx';
