...
@Component({
  ...
  styleUrls: ['./price-wise-purchase.component.scss']
})
export class PriceWisePurchaseComponent implements OnInit {
  loader = true;
  ...
  search() {
    if (this.titlee === "") {
      ...
      this.priceWisePurchaseList = this.priceWisePurchaseList.filter((res: any) => {
  ...
  oncheckBrand(data: any) {
    console.log(data);
    this.dataId = data;
    this.priceWisePurchaseForm.patchValue({ brand: this.dataId });
  ...
  getSelectedPriceWisePurchaseDates() {
    console.log(this.priceWisePurchaseForm.value);
    const start = this.datepipe.transform(this.priceWisePurchaseForm.value.start, 'yyyy-MM-dd');
    const end = this.datepipe.transform(this.priceWisePurchaseForm.value.end, 'yyyy-MM-dd');
    console.log(start);
    console.log(end);
 ...
 async generatePDFAgain() {
  ...
  const permanentContent = 'Price Wise Purchase Report';
  doc.setFontSize(16);
  ...
  doc.text(`From Date: ${this.formatDate(this.priceWisePurchaseForm.get('start').value)}`, 14, 45);
  doc.text(`User: ${result?.role}`, (pageWidth - textWidth+20), 33);
  doc.text(`Print Date: ${printDate}`, (pageWidth - textWidth+20), 39);
  doc.text(`To Date: ${this.formatDate(this.priceWisePurchaseForm.get('end').value)}`, (pageWidth - textWidth+20), 45);
