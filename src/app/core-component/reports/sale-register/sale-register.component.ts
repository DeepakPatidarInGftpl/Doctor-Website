...
export class SaleRegisterComponent implements OnInit {
  ...
  getproductDayBook() {
    this.reportService.getSaleRegister(this.startDate, this.endDate, this.fyID, this.selectData).subscribe((res: any) => {
      console.log(res);
 ...
 async generatePDFAgain() {
  ...
  doc.text(`To Date: ${this.formatDate(this.productDayBookform.get('end').value)}`, (pageWidth - textWidth), 45);
    ...
    autoTable(doc, {
      head: [
        ['#', 'Bill Date', 'Customer Bill No.', 'Total Discount', ' SubTotal', 'RoundOff', 'Total', 'Additional Charges', 'Taxable Amount', 'Total Tax', 'Gst']
      ],
  ...
  exportToExcel(): void {
    ...
    const fileName = 'saleregister.xlsx';
