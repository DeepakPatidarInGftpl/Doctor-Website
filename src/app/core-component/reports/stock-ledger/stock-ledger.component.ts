...
export class StockLedgerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.commonService.validateAndClearDates(this.stockLedgerform, this.minDate, this.maxDate);

    this.startDate = this.stockLedgerform.value?.start;
    this.endDate = this.stockLedgerform.value?.end;
    this.productId = this.stockLedgerform.value?.productId;
  ...
  //product name
  private _filter2(item_code: string): any[] {
  ...
  displayFn2(user: any): string {
    return user && user?.item_code || user?.name ? user?.item_code || user?.name : '';
  }
  suppliers: any[] = [];
  getProduct() {
  ...
  search() {
    if (this.titlee === "") {
      ...
      this.stockLedgerList = this.stockLedgerList.filter((res: any) => {
  ...
  stockLedger: any
  getStockLedger() {
    this.reportService.getStockLedger(this.startDate, this.endDate, this.productId, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
      this.stockLedger = res;
  ...
  oncheckAccount(data: any) {
    console.log(data);
    this.dataId = data;
    this.stockLedgerform.patchValue({ productId: this.dataId });
    console.warn(this.stockLedgerform.value);
    this.productId = this.stockLedgerform.value?.productId;
    this?.getStockLedger();
  }
  getSelectedstockLedgerDates() {
    console.log(this.stockLedgerform.value);
    const start = this.datepipe.transform(this.stockLedgerform.value.start, 'yyyy-MM-dd');
    const end = this.datepipe.transform(this.stockLedgerform.value.end, 'yyyy-MM-dd');
    console.log(start);
    console.log(end);
  ...
  getProductoncheckAccount(data: any) {
    this.stockLedgerform.patchValue({ productName: data });
    console.warn(this.stockLedgerform.value);
    this?.getStockLedger();
  }
  // convert to pdf
  ...
  generatePDFAgain() {
    ...
    autoTable(doc, {
      head: [
        ['#', 'Date', 'Voucher Type', 'Voucher No.', 'In Qty', 'Out Qty', 'Price']
      ],
      body: this.stockLedgerList.map((row: any, index: number) => [
        index + 1,
  ...
  exportToExcel(): void {
    ...
    const fileName = 'StockLedger.xlsx';
  ...
  changePg(val: any) {
    console.log(val);
    if (val == -1) {
      this.itemsPerPage = this.stockLedgerList?.length;
    }
  }
  //23-5
  branchList: any[] = [];
