...
export class CategoryWiseSaleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.commonService.validateAndClearDates(this.categoryWiseSaleform, this.minDate, this.maxDate);
  ...
  getCategoryWiseSale() {
    ...
    this.reportService.getCategoryWiseSale(this.startDate, this.endDate, this.category, this.subcategory, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
 ...
 async generatePDFAgain() {
    ...
    try {
      ...
      doc.setFontSize(25);
      doc.text('Category Wise Sale Report', 52, 25);
      // Add details
      doc.setFontSize(12);
      doc.text(`Business Location: ${result?.branch}`, 14, 39);
      doc.text(`From Date: ${this.formatDate(this.categoryWiseSaleform.get('start').value)}`, 14, 45);
      doc.text(`User: ${result?.role}`, 172, 33);
      doc.text(`Print Date: ${printDate}`, 153, 39);
      doc.text(`To Date: ${this.formatDate(this.categoryWiseSaleform.get('end').value)}`, 157, 45);
      autoTable(doc, {
        head: [
