...
export class AbcAnalysisComponent implements OnInit {
  ...
  getStockAlert() {
    ...
    this.reportService.getAbcAnalysis(this.startDate, this.endDate, this.category, this.subcategory, this.brand, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
      this.stockAlert = res;
  ...
  generatePDFAgain() {
    const doc = new jsPDF();
    const subtitle = 'PV';
    const title = 'Abc Analysis Report';
    ...
    autoTable(doc, {
      head: [
        ['#', 'Product Name', 'Variant', 'No.Of Bills', 'Sale Amount', 'Sale(%)', 'Grade']
      ],
  ...
  exportToExcel(): void {
    ...
    const fileName = 'abcanalysis.xlsx';
