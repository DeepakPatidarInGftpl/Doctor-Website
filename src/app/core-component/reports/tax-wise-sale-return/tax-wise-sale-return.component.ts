...
@Component({
  selector: 'app-tax-wise-sale-return',
  templateUrl: './tax-wise-sale-return.component.html',
  styleUrls: ['./tax-wise-sale-return.component.scss']
})
export class TaxWiseSaleReturnComponent implements OnInit {
  ...
  getPurchaseRegister() {
    this.reportService.getTaxWiseSaleReturn(this.startDate, this.endDate, this.fyID, this.selectData).subscribe((res) => {
      console.log(res);
 ...
 async generatePDFAgain() {
  ...
  const permanentContent = 'Tax Wise Sale Return Report';
  doc.setFontSize(16);
  ...
  doc.text(`To Date: ${this.formatDate(this.purchaseRegisterForm.get('end').value)}`, (pageWidth - textWidth+20), 45);
    ...
    doc.save('Tax_wise_Sale_Return .pdf');
  }
  ...
  exportToExcel(): void {
    ...
    const fileName = 'taxwisesalereturn.xlsx';
