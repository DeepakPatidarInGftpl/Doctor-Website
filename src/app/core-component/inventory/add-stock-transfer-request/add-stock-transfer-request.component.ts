...
@Component({
  selector: 'app-add-stock-transfer-request',
  templateUrl: './add-stock-transfer-request.component.html',
  styleUrls: ['./add-stock-transfer-request.component.scss']
})
export class AddStockTransferRequestComponent implements OnInit {
  ...
  prefixNo: any;
  getprefix() {
    this.stockService.getStockTransferRequestPrefix().subscribe((res: any) => {
      console.log(res);
      if (res.success == true) {
  ...
  stockTransferDateValidation(financialYear) {
    const dateControl = this.stockTransferRequestForm.get('request_date');
  ...
  submit() {
    ...
    if (this.stockTransferRequestForm.valid) {
      ...
      this.stockService.addStockTransferRequest(formdata).subscribe(res => {
        console.log(res);
        this.getRes = res;
