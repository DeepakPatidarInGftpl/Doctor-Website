...
@Component({
  selector: 'app-pos-add-expense-voucher',
  templateUrl: './pos-add-expense-voucher.component.html',
  styleUrls: ['./pos-add-expense-voucher.component.scss']
})
export class PosAddExpenseVoucherComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.expensevoucherForm = this.fb.group({
      ...
      created_from: new FormControl('POS',),
  ...
  submit() {
    ...
    if (this.expensevoucherForm.valid) {
      ...
      formdata.append('total_amount', this.expensevoucherForm.get('total_amount')?.value.toFixed(2));
      ...
      this.posCartService.addPosExpenseVoucher(formdata).subscribe((res: any) => {
        this.loaders = false;
        this.addRes = res
        if (this.addRes.success) {
          this.toastrService.success(this.addRes.msg)
          this.expensevoucherForm.reset();
          this.fromPartyControl.setValue('');
