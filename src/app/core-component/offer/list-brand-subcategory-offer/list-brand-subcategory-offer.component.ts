...
@Component({
  selector: 'app-list-brand-subcategory-offer',
  templateUrl: './list-brand-subcategory-offer.component.html',
  styleUrls: ['./list-brand-subcategory-offer.component.scss']
})
export class ListBrandSubcategoryOfferComponent implements OnInit {
...
userDetails:any;
  ngOnInit(): void {
    this.offerService.getDiscount().subscribe((res:any) => {
      // console.log(res);
      res.forEach((res: any) => {
        if (res.discount_type === 'BasedOnSubCategorie’sBrand') {
...
select=false
 ...
 generatePDFAgain() {
  ...
  const subtitle = 'PV';
  const title = 'Brand Subcategory Offer';
  ...
  doc.save('Brand_Subcategory_offer.pdf');
}
