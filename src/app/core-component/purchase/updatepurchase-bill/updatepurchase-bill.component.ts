...
export class UpdatepurchaseBillComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.puchaseBillForm = this.fb.group({
      ...
      supplier_bill_date: new FormControl(defaultDate, [Validators.required]),
  ...
  udateCart(add: any): FormArray {
    console.log(add);
    const formarr = this.puchaseBillForm.get('purchase_bill') as FormArray;
    ...
    if (event.batch.length > 0) {
      ...
      if (event?.product?.purchase_tax_including == true) {
        ...
        barcode.patchValue({
          ...
          discount: (event.batch[0]?.discount && event.batch[0]?.discount?.length > 0) ? event.batch[0]?.discount : 0,
