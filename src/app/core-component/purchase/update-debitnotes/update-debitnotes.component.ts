...
export class UpdateDebitnotesComponent implements OnInit {
  ...
  udateCart(add: any): FormArray {
    ...
    const formarr = this.debitNotesForm.get('cart') as FormArray;
    add.forEach((j: any, i) => {
      ...
      formarr.push(this.fb.group({
        ...
        mrp: j.mrp,
        deduction: j.deduction == null ? 0 : j.deduction,
        tax: j.tax || 0,
  ...
  private _filter(value: string | number, include: boolean): any[] {
    ...
    const filteredSuppliers = include
      ? this.suppliers.filter(supplier => supplier.company_name.toLowerCase().includes(filterValue) || supplier.company_name.toLowerCase().includes(filterValue))
      : this.suppliers.filter(supplier => !supplier.company_name.toLowerCase().includes(filterValue) || supplier.company_name.toLowerCase().includes(filterValue));
