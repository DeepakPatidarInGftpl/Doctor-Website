...
public generateNewPdf(obj : any) {
  ...
  if (data.cell.colSpan === 2 || [0, 1, 2, 3].includes(data.row.index)) {
   ...
   if (obj.otherArray.length > 1) {

    for (let index = 1; index < obj.otherArray.length; index++) {
    // console.log(index,'obj.otherArray')
    pdf.addPage()
    let arrs :any[][] = obj.otherArray[index].map((cart : any, n:number) => {
